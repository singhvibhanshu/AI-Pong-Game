...
def run_neat(config):
    # load the checkpoint instead of starting from scratch
    checkpoint_file = 'neat-checkpoint-45'
...
def play_against_checkpoint(config, checkpoint_file="neat-checkpoint-45"):
    # Restore the saved population from the checkpoint
    p = neat.Checkpointer.restore_checkpoint(checkpoint_file)
    
    # Get the best genome (highest fitness) from the population
    best_genome = None
    ...
    if best_genome is None:
        print("No valid genome found in the checkpoint!")
        return
...
if __name__ == '__main__':
    ...
    play_against_checkpoint(config, "neat-checkpoint-45")